...
```mermaid
graph TD
    subgraph TMM["Tool & MCP Manager"]
        direction TB
        REG["MCP Server<br/>Registry"]
        CAT["Tool<br/>Catalog"]
        ASN["Assignment Engine<br/><small>Least Privilege</small>"]
        LC["Lifecycle Controller<br/><small>Health Checks / Versioning / Deprecation / Caching</small>"]
        SEC["Security Layer<br/><small>before_tool_callback / Output Sanitization / Permission Eval</small>"]
    ...
    SEC --> STDIO["STDIO Servers<br/><small>local process</small>"]
    SEC --> HTTP["HTTP+SSE Servers<br/><small>remote service</small>"]
    SEC --> OBS["Observability Platform<br/><small>traces, metrics, logs</small>"]
    ...
    class REG,CAT,ASN core
    class LC,SEC infra
    class STDIO,HTTP external
...
```mermaid
graph TD
    PLAT["Platform Level<br/><small>Global tool policies: banned tools,<br/>global rate limits, data classification ceilings</small>"]
    PLAT --> T1["Team Level<br/><small>Team-scoped tool allowlist</small>"]
    PLAT --> T2["Team Level<br/><small>Different team, different allowlist</small>"]
    T1 --> A["Agent A<br/><small>tools ⊂ team</small>"]
    T1 --> B["Agent B<br/><small>tools ⊂ team</small>"]
    T2 --> C["Agent C<br/><small>tools ⊂ team</small>"]
    T2 --> D["Agent D<br/><small>tools ⊂ team</small>"]
...
stateDiagram-v2
    [*] --> Unknown
    Unknown --> Active : register()
    Active --> Degraded : health check fails x unhealthy_threshold
    Degraded --> Active : recovers (healthy_threshold met)
    Degraded --> Unhealthy : continues failing
    Degraded --> Deregistered : all health checks fail > max_downtime
    Unhealthy --> Active : manual action or auto-recovery
    Active --> Deprecated : deprecate()
    Deprecated --> Deregistered : deregister()

    note right of Active : healthy_threshold met
    note right of Deprecated : tools still work but<br/>new assignments blocked
    note right of Deregistered : all tool bindings removed
```
...
```mermaid
graph LR
    subgraph SEC["Security Enforcement Points"]
        direction LR
        subgraph BEFORE["BEFORE"]
            direction TB
            B1["Assignment check<br/><small>Least Privilege</small>"]
            B2["Rate limiting"]
            B3["Policy eval<br/><small>p. 292</small>"]
            B4["HITL gate for<br/>irreversible tools<br/><small>p. 91</small>"]
        end
        subgraph DURING["DURING"]
            direction TB
            D1["Schema validation<br/>on server side"]
            D2["Timeout<br/>enforcement"]
            D3["Sandboxed<br/>execution"]
        end
        subgraph AFTER["AFTER"]
            direction TB
            A1["Output<br/>sanitization<br/><small>untrusted</small>"]
            A2["PII scan"]
            A3["Size limit"]
            A4["Injection<br/>detection"]
        end
    end
