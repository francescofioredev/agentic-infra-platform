...
```mermaid
graph TD
    subgraph GS["Guardrail System"]
        direction TB
        PR["Policy Registry<br/><small>Policy schemas, severity rules,<br/>action configs</small>"]
        GA["Guardrail Agents<br/><small>Per-level monitors<br/>with LLM evaluators</small>"]
        HITL["HITL Escalation Manager<br/><small>Routing, timeout,<br/>safe defaults</small>"]
        MP["Monitoring Pipeline<br/><small>Input validation, before_tool_callback, output filtering,<br/>jailbreak detection, checkpoint/rollback engine</small>"]
        ALS["Alert & Logging Subsystem<br/><small>Safety intervention log, alert routing, metric emission</small>"]
    ...
    class PR,GA,HITL guardrail
    class MP infra
    class ALS foundational
```
...
```mermaid
graph TD
    GGA["Global Guardrail Agent<br/><small>Platform-wide policies<br/>Monitors Level 0 orchestrator decisions</small>"]
    GGA --> TGA["Team Alpha<br/>Guardrail Agent<br/><small>Team-level policies</small>"]
    GGA --> TGB["Team Beta<br/>Guardrail Agent"]
    GGA --> TGC["Team Gamma<br/>Guardrail Agent"]
    TGA --> A1G["A1-G"]
    TGA --> A2G["A2-G"]
    TGA --> A3G["A3-G"]
    ...
    class GGA guardrail
    class TGA,TGB,TGC core
    class A1G,A2G,A3G guardrail
```
...
```mermaid
graph LR
    AR["Agent Runtime<br/><small>Emits events:<br/>tool_call, tool_result,<br/>agent_output, state_change, error</small>"]
    EB["Event Bus<br/><small>Redis Streams<br/>Channels: guardrail.global,<br/>guardrail.team.*, guardrail.agent.*</small>"]
    GA["Guardrail Agent<br/><small>Subscriber<br/>Evaluates against policy set<br/>Emits: evaluation, intervention</small>"]

    AR -->|"events"| EB -->|"subscribe"| GA
...
graph TD
    ...
    SC --> TIMEOUT["Timeout"]
    SUCCESS --> ALLOW["ALLOW"]
    FAILURE --> BLOCK1["BLOCK<br/><small>log + alert</small>"]
    TIMEOUT --> BLOCK2["BLOCK<br/><small>log + alert</small>"]

    classDef check fill:#7B68EE,stroke:#5A4FCF,color:#fff
    classDef allow fill:#2ECC71,stroke:#1FA855,color:#fff
    classDef block fill:#E74C3C,stroke:#C0392B,color:#fff
    classDef neutral fill:#95A5A6,stroke:#7F8C8D,color:#fff

    class SC check
    class SUCCESS,FAILURE,TIMEOUT neutral
    class ALLOW allow
...
```mermaid
graph LR
    RTS["Red-Team Test Suite"] --> APG
    APG["Attack Payload Generator<br/><small>Known CVEs, Novel attacks, Fuzzing</small>"]
    APG -->|"payloads"| GS["Guardrail System<br/><small>Test Mode<br/>Same pipeline, no side effects</small>"]
    GS -->|"results"| RA["Result Analyzer<br/><small>Metrics: Detection rate,<br/>False positive, Latency</small>"]
