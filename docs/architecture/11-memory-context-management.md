...
```mermaid
graph TD
    subgraph MCM["Memory & Context Management Subsystem"]
        direction TB
        subgraph MM["Memory Manager (Core)"]
            direction TB
            SM["Session Memory<br/><small>(per-turn)</small>"]
            WM["Working Memory<br/><small>(state)</small>"]
            LTM["Long-Term Memory<br/><small>(persist)</small>"]
            TM["Team Memory<br/><small>(shared)</small>"]
            SSL["State Store Layer<br/><small>prefix: temp: | agent: | team: | user: | app:</small>"]
            SM --> SSL
            WM --> SSL
            LTM --> SSL
            TM --> SSL
        end
        CO["Context Optimizer<br/><small>Pruning, Summarization,<br/>Compression, Windowing,<br/>Prioritization</small>"]
        RAG["RAG Engine<br/><small>Ingestion, Retrieval,<br/>Hybrid Search, GraphRAG,<br/>Agentic RAG</small>"]
        MLC["Memory Lifecycle Manager<br/><small>TTL enforcement,<br/>Invalidation, GC / Compaction,<br/>Privacy compliance</small>"]
    end
    MCM --> Redis["Redis<br/><small>(hot)</small>"]
    MCM --> VS["Vector Store"]
    MCM --> PG["PostgreSQL<br/><small>(durable)</small>"]
    ...
    class SM,WM,LTM,TM core
    class SSL,CO,RAG,MLC infra
    class Redis,VS,PG external
```
...
```mermaid
graph TD
    L4["<b>Layer 4: KNOWLEDGE MEMORY</b><br/><small>cross-system, RAG-backed</small><br/><br/>Scope: Platform-wide or per-domain<br/>Storage: Vector store + document store<br/>TTL: Source-dependent (hours to indefinite)<br/>Examples: Product catalog, company policies, API docs"]
    L3["<b>Layer 3: LONG-TERM MEMORY</b><br/><small>cross-session, persistent</small><br/><br/>Scope: Per-user (user:) or per-app (app:)<br/>Storage: PostgreSQL + optional vector embeddings<br/>TTL: 30-365 days (configurable per data type)<br/>Examples: User preferences, conversation summaries"]
    L2["<b>Layer 2: WORKING MEMORY</b><br/><small>cross-turn, session-scoped</small><br/><br/>Scope: Per-agent (agent:) or per-team (team:)<br/>Storage: Redis<br/>TTL: Session end<br/>Examples: Task state, intermediate results, plan progress"]
    L1["<b>Layer 1: SESSION MEMORY</b><br/><small>per-turn, ephemeral</small><br/><br/>Scope: Per-interaction (temp:)<br/>Storage: In-process memory<br/>TTL: Current turn only<br/>Examples: Current message, current tool call, reasoning trace"]

    L4 --> L3 --> L2 --> L1

    classDef knowledge fill:#7B68EE,stroke:#5A4FCF,color:#fff
    classDef longterm fill:#4A90D9,stroke:#2C5F8A,color:#fff
    classDef working fill:#F39C12,stroke:#D68910,color:#fff
    classDef session fill:#2ECC71,stroke:#1FA855,color:#fff
...
graph TD
    subgraph Agent["Agent: ResearchAgent"]
        direction TB
        SES["<b>Session Memory (temp:)</b><br/><small>Current user message<br/>Current reasoning trace<br/>Current tool call context</small>"]
        WRK["<b>Working Memory (agent:)</b><br/><small>Scratchpad / notes<br/>Intermediate results<br/>Conversation buffer (last N)<br/>Active plan + progress<br/>Tool result cache</small>"]
        LNG["<b>Long-Term Memory (user:)</b><br/><small>User preference profile<br/>Past interaction summaries<br/>Learned patterns</small>"]
        KNW["<b>Knowledge Access (knowledge:)</b><br/><small>RAG retrieval interface<br/>Domain knowledge queries</small>"]
        SES --> WRK --> LNG --> KNW
    end
...
```mermaid
graph TD
    subgraph TeamAlpha["Team Alpha Shared Memory"]
        direction TB
        TWM["<b>Team Working Memory (team:alpha:*)</b><br/><small>team:alpha:active_plan → Current plan + progress<br/>team:alpha:shared_context → Shared facts/findings<br/>team:alpha:coordination → Who is doing what<br/>team:alpha:decisions → Key decisions + rationale<br/>team:alpha:blockers → Current blocking issues</small>"]
        TKB["<b>Team Knowledge Base (team:alpha:kb:*)</b><br/><small>team:alpha:kb:domain_facts → Accumulated domain knowledge<br/>team:alpha:kb:past_tasks → Summaries of completed tasks<br/>team:alpha:kb:lessons → Learned patterns/mistakes</small>"]
        TAC["<b>Access Control</b><br/><small>Supervisor → read/write all team:alpha:* keys<br/>Workers → read all, write only own namespace<br/>Guardrails → read-only (for monitoring)<br/>Other teams → no access (isolation)</small>"]
        TWM --> TKB --> TAC
    end
...
```mermaid
graph TD
    subgraph FullCtx["Full Agent Context<br/><small>(potentially unlimited)</small>"]
        direction TB
        K1["System Prompt — KEEP"]
        K2["Current User Message — KEEP"]
        K3["Active Plan + Current Step — KEEP"]
        K4["Recent Conversation (last 5) — KEEP"]
        S1["Older Conversation (turns 6-20) — SUMMARIZE"]
        P1["Previous Tool Results — PRUNE if unused"]
        P2["Failed Reasoning Attempts — PRUNE"]
        P3["Stale Shared Context — PRUNE"]
        D1["Redundant Information — DEDUPLICATE"]
        DR1["Very Old History (turns 20+) — DROP or ARCHIVE"]
    end

    FullCtx -->|"Context Optimizer"| OptCtx

    subgraph OptCtx["Optimized Context<br/><small>~5000 / 8000 budget — OK</small>"]
        direction TB
        O1["System Prompt — 800 tk"]
        O2["Current User Message — 200 tk"]
        O3["Active Plan + Current Step — 300 tk"]
        O4["Recent Conversation (5 turns) — 1500 tk"]
        O5["Summary of turns 6-20 — 400 tk"]
        O6["Relevant RAG chunks — 800 tk"]
        O7["Relevant Tool Results — 500 tk"]
        O8["Team Shared Context (filtered) — 500 tk"]
    end

    classDef keep fill:#2ECC71,stroke:#1FA855,color:#fff
    classDef prune fill:#E74C3C,stroke:#C0392B,color:#fff
    classDef summarize fill:#F39C12,stroke:#D68910,color:#fff
    classDef optimized fill:#4A90D9,stroke:#2C5F8A,color:#fff

    class K1,K2,K3,K4 keep
    class S1 summarize
    class P1,P2,P3,D1,DR1 prune
    class O1,O2,O3,O4,O5,O6,O7,O8 optimized
```
...
graph TD
    subgraph RAGEngine["RAG Engine"]
        direction TB
        IP["Ingestion Pipeline<br/><small>Documents → Chunk → Embed → Store (p. 217)</small>"]
        IP --> KS
        subgraph KS["Knowledge Stores"]
            direction LR
            PWK["Platform-wide Knowledge<br/><small>knowledge:*<br/>Product docs, Policies,<br/>API refs, FAQ</small>"]
            TSK["Team-scoped Knowledge<br/><small>team:X:kb:*<br/>Task history, Domain facts,<br/>Lessons, Decisions</small>"]
            ASK["Agent-specific Knowledge<br/><small>agent:X:kb<br/>Personal notes,<br/>Learned prefs</small>"]
        end
        KS --> RP
        RP["Retrieval Pipeline<br/><small>Query → Embed → Hybrid Search → Re-rank (p. 222-223)</small>"]
        RP --> ARC
        ARC["Agentic RAG Controller<br/><small>Agent decides WHAT, WHEN, WHERE to retrieve (p. 227)</small>"]
    end
    ...
    class IP,RP,ARC core
    class PWK,TSK,ASK infra
```
